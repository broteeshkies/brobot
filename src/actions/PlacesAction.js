...
const restrictedCities = ['Солт Лейк Сити'];

const restrictedTranslates = ['СЫСЕРТЬ'];

const eatPhrases = ['поесть в', 'еда в', 'пообедать в', 'покушать в'];

const drinkPhrases = ['нажраться в', 'бухать в', 'выпить в'];

const coffeePhrases = ['выпить кофе в', 'попить кофе в', 'напитки в', 'кофе в', 'кофейни в'];

const shopsPhrases = ['магазы в', 'магазины в', 'затариться в', 'товары в'];

const artsPhrases = ['насладиться в', 'посмотреть в', 'искусство в'];

const sightsPhrases = ['достопримечательности в', 'интересное в', 'интересности в', 'туристам в'];

const outdoorsPhrases = ['сходить в', 'пойти в', 'найти в', 'собраться в'];
...
export default class PlacesAction extends Action {
    ...
    getSection(action) {
        ...
        if (eatPhrases.includes(action)) {
            section = categories[0];
        }
        if (drinkPhrases.includes(action)) {
            section = categories[1];
        }
        if (coffeePhrases.includes(action)) {
            section = categories[2];
        }
        if (shopsPhrases.includes(action)) {
            section = categories[3];
        }
        if (sightsPhrases.includes(action)) {
            section = categories[4];
        }
        if (outdoorsPhrases.includes(action)) {
        ...
        try {
            ...
            const caption =
                ...
                venues
                    .map(
                        (venue, index) =>
                            ` ${index + 1}. <i>${venue.name}</i>${venue.address ? ` ${venue.address}` : ''} `,
                    )
                    .join('');
            return {
                photos,
    ...
    async prepareCityName(_name) {
        ...
        if (restrictedCities.includes(name)) {
            return name;
        }
        if (userCitiesEnum[name]) {
            name = userCitiesEnum[name];
        }
