...
export default class Action {
    ...
    async repost({ message, chatId, forwardFrom }) {
        ...
        this.log('НАТА РЕАЛИЗУЙ МЕНЯ', message);
        return null;
    }
    ...
    send(msg, text, params) {
        const { delay = random(0, 5, 1), reply = 50, method = 'sendMessage' } = params;
        const chatId = msg.chat.id || msg.from.id;
        const bot = this.bot;
        const opt = this.percentProbability(reply) ? { reply_to_message_id: msg.message_id } : {};
    ...
    deleteMessage(chatId, messageId, params = {}) {
        this.bot.deleteMessage(chatId, messageId, params);
    }
